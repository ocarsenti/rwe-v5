...
Résultat sur les 3 mécanismes couverts : 1 confirmé mot pour mot, 1 manqué par le moteur, 1
détecté en interne mais pas encore assez fort pour faire bouger le verdict global. Le détail en
image.
...
mécanisme), ici le **verdict global** du moteur (REJETÉ) matche directement la vraie décision
HAS (SA Insuffisant) — porté par la structure causale CIRCULAR, pas par un flag isolé. Bon
exemple pédagogique de pourquoi le verdict combine structure + sévérité des biais plutôt que de
reposer sur un seul flag (cf. recalibrage `assess_methodological_risk`, [[project_rwe_v5]]).
...
d'identifier proprement l'effet du dispositif. Son verdict global : REJETÉ. Exactement la
décision réelle de la HAS (Service Attendu Insuffisant).

Le détail mécanisme par mécanisme est plus nuancé : un des biais individuels ne se retrouve pas
clairement dans le texte de l'avis, et l'alignement dispositif/population n'était pas assez
alarmant pour peser seul sur le score. C'est précisément pour ça que le verdict global ne repose
jamais sur un seul flag isolé : il regarde d'abord si la structure elle-même tient debout.
...
| MF_D pertinence de l'évidence | Alignement jugé correct (score 0,9), aucune alerte forte | « Aucune individualisation des résultats en fonction de la localisation de l'AVC […] l'utilisation du cathéter dans l'artère vertébrale ne correspond pas à l'indication du marquage CE. » | Manqué |

MF_C/MF_E : hors périmètre, absents de la grille. **Nouveau, spécifique à ce cas** : un bandeau
"Verdict global du moteur = Décision réelle HAS" en tête d'image, avant la grille de détail —
élément visuel qui n'existait pas dans le template du cas 1, à garder pour les cas futurs où le
verdict global (pas seulement un mécanisme isolé) matche la décision réelle.
...
- **TRIPLE ACTION (7620)** — rejet HAS, 2 confirmations textuelles (NO_COMPARATOR + CAS_CONTEXT),
  fait partie des "5 trop indulgents" documentés — vérifier son `overall_verdict` avant de
  l'utiliser (structure DIRECT, donc probablement encore ACCEPTABLE malgré le rejet réel — bon
